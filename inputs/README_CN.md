...
抽取器可以从 `inputs/prd/`、`inputs/notes/`、`inputs/context/` 中直接读取以下文本型需求来源：

- Markdown 和文本：`.md`、`.markdown`、`.txt`
- 结构化文本：`.json`、`.yaml`、`.yml`
- 网页导出：`.html`、`.htm`
- 表格：`.csv`、`.tsv`
- Word 文档：`.docx`
- Excel 工作簿：`.xlsx`；当前 Python 环境安装 `xlrd` 时也支持 `.xls`

旧版 `.doc` 文件不会被直接解析，建议先转换为 `.docx`，这样可以避免静默乱码、丢行或抽取结果不稳定。

推荐放置方式：

- 正式 PRD、Word 需求文档、Excel 需求表、CSV 需求列表放入 `inputs/prd/`。
- 会议纪要、补充说明、澄清表格放入 `inputs/notes/`。
- API 表、权限矩阵、状态表、集成依赖说明放入 `inputs/context/`。
