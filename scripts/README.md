...
Most users will interact with this directory through `run_pipeline.py`, but each file can also be used independently for debugging, tuning, validation, generation, archive workflows, and downstream context-pack assembly.
...
## Platform Principles Reflected Here

These scripts follow the same platform model described across the project docs:

- structure understanding first
- validate before downstream generation
- keep evidence, confidence, and unknowns visible
- assemble context packs only after the core artifacts are stable
...
| `build_context_pack.py` | Builds ready-to-copy Markdown context packs for OpenSpec, Superpowers, and AI-oriented downstream workflows. | `python scripts/build_context_pack.py --target openspec --change-name demo --domain account --title "Demo"` |
...
python scripts/build_context_pack.py --target superpowers --change-name <change-name> --domain <domain> --title "<title>" --goal "Implementation planning and support"
...
In practice, `run_pipeline.py` covers steps 1 to 6 for you. Build context packs when you need a downstream handoff package.
