...
class ExtractScreenshotEvidenceTests(unittest.TestCase):
    def test_write_screenshot_outputs_builds_text_and_component_artifacts(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            (screenshots / "login-screen.txt").write_text("登录页\n手机号\n验证码\n登录\n注册", encoding="utf-8")

            write_screenshot_outputs(workspace)

            text_payload = json.loads((working / "screenshot-text-evidence.json").read_text(encoding="utf-8"))
            ...
            self.assertEqual(1, len(text_payload["screenshots"]))
            self.assertIn("登录页", text_payload["screenshots"][0]["text_evidence"])
